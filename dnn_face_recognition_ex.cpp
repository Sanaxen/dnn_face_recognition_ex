...
int main(int argc, char** argv) try
{
	...
	if (std::string(argv[1]) == "--recog")
	{
		...
		do {
			std::string user_name = webcam_face_recognition(detector, sp, net, 0);
			printf("user %s\n", user_name.c_str());
			...
			if (user_name != "" && user_name != "unknown")
			{
				...
				try
				{
					...
					if (!user.empty())
					{
						sc::myCV::putText_Jpn(user, (filename + "Ç≥ÇÒÇ≈Ç∑").c_str(), cv::Point(50, 90), std::string("ÇlÇr ÉSÉVÉbÉN").c_str(), 0.5, cv::Scalar(200, 0, 0), 3);
	...
	if (std::string(argv[1]) == "--image")
	{
		...
		std::string user_name = face_recognition(face_image, detector, sp, net, shapelist, shapevalue_list);
		printf("user %s\n", user_name.c_str());
	...
	if (std::string(argv[1]) == "--test")
	{
		...
		for (int i = 0; i < shapelist.size(); i++)
		{
			...
			if (user_name != "" && user_name != "unknown")
			{
				...
				if (!user.empty())
				{
					sc::myCV::putText_Jpn(user, (filename + "Ç≥ÇÒÇ≈Ç∑").c_str(), cv::Point(10, 90), std::string("ÇlÇr ÉSÉVÉbÉN").c_str(), 0.5, cv::Scalar(200, 0, 0), 3);
